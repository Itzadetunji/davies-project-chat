			...
			<main className="relative flex h-[100svh] w-full flex-col">
				...
				<ScrollArea className="flex-end flex flex-1 flex-col items-baseline justify-end overflow-y-scroll p-4">
					{messages.map(({ role, content, image }, index) => (
						<div
							key={index}
							className={`my-2 rounded-l-lg rounded-t-lg p-4 ${role === "user" ? "ml-auto w-fit max-w-[80%] justify-end bg-peach text-left text-white" : "justify-start bg-white text-right text-black"}`}
						>
							{content}
							{image && (
								<div className="mt-2">
